    ...
    public void execute() {
        try {
            ...
            if (!getSessionDir().getParentFile().exists()) {
                ...
                if (separatePetMr) {
                    ...
                    if (sessionXml.exists()) {
                        ...
                        if (bean instanceof XnatPetmrsessiondataBean) {
                            ...
                            PrearcUtils.resetStatus((XDATUser) getUser(), getSessionData().getProject(), getSessionData().getTimestamp(), getSessionData().getFolderName(), true);
                ...
                final PrearcImporterA.PrearcSession session = new PrearcImporterA.PrearcSession(getSessionData().getProject(), getSessionData().getTimestamp(), getSessionData().getFolderName(), null, (XDATUser) getUser());
                final FinishImageUpload uploader = new FinishImageUpload(null, (XDATUser) getUser(), session, null, false, true, false);
