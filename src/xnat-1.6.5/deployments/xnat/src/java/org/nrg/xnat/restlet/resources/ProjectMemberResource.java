				...
				if(user.canDelete(proj)){
					try {
						for(XDATUser newUser: newUsers){
						    proj.removeGroupMember(group.getId(), newUser, user,newEventInstance(EventUtils.CATEGORY.PROJECT_ACCESS, EventUtils.REMOVE_USER_FROM_PROJECT));
						}
					} catch (Exception e) {
						logger.error("",e);
					}
					...
					if (unknown.size()>0){
                        ...
                        try {
							for(String uID : unknown){
							    ...
							    //SEND email to user
							    final PersistentWorkflowI wrk=PersistentWorkflowUtils.getOrCreateWorkflowData(null, user, XnatProjectdata.SCHEMA_ELEMENT_NAME, proj.getId(), proj.getId(), newEventInstance(EventUtils.CATEGORY.PROJECT_ACCESS, EventUtils.INVITE_USER_TO_PROJECT));
						    	try {
