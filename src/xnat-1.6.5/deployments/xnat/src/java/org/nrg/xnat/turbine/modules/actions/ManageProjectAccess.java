        ...
        if (owners!=null){
            ...
            for (String newOwner: currentOwners){
                if (!ownersL.contains(newOwner))
                {
                    ...
                    for (XdatUser newU : al){
                        XDATUser newUOM = new XDATUser(newU);
                        project.removeGroupMember(project.getId() + "_" + BaseXnatProjectdata.OWNER_GROUP, newUOM, TurbineUtils.getUser(data),newEventInstance(data,EventUtils.CATEGORY.PROJECT_ACCESS,EventUtils.REMOVE_USER_FROM_PROJECT));
                    }
                }
            }
        }else{
        ...
        if (members!=null){
            ...
            for (String newMember: currentMembers){
                if (!membersL.contains(newMember))
                {
                    ...
                    for (XdatUser newU : al){
                        XDATUser newUOM = new XDATUser(newU);
                        project.removeGroupMember(project.getId() + "_" + BaseXnatProjectdata.MEMBER_GROUP, newUOM, TurbineUtils.getUser(data),newEventInstance(data,EventUtils.CATEGORY.PROJECT_ACCESS,EventUtils.REMOVE_USER_FROM_PROJECT));
                    }
                }
            }
        }else{
        ...
        if (collaborators!=null){
            ...
            for (String newMember: currentMembers){
                if (!collaboratorsL.contains(newMember))
                {
                    ...
                    for (XdatUser newU : al){
                        XDATUser newUOM = new XDATUser(newU);
                        project.removeGroupMember(project.getId() + "_" + BaseXnatProjectdata.COLLABORATOR_GROUP, newUOM, TurbineUtils.getUser(data),newEventInstance(data,EventUtils.CATEGORY.PROJECT_ACCESS,EventUtils.REMOVE_USER_FROM_PROJECT));
                    }
                }
            }
        }else{
