...
//    }
	...
	*/
	...
	private Collection<String> validateElements(Map<String,String> elements, Collection<String> aliases) throws Exception {
        if (PoolDBUtils.HackCheck(elements.values())) {
            final String values = Joiner.on(", ").join(elements.values());
        ...
        if (PoolDBUtils.HackCheck(aliases)) {
            final String values = Joiner.on(", ").join(aliases);
            logger.error("Found possible SQL injection attempt in values: " + values);
