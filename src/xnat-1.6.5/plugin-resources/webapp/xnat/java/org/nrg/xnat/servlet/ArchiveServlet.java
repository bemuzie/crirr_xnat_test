...
public class ArchiveServlet extends HttpServlet {
    ...
    protected void getCatalog(XDATUser user, String path, HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException {
        ...
        if (rootElementName.equals("stored")) {
            File f = user.getCachedFile("catalogs/" + path);
            if (f.exists()) {
                writeFile(f, res);
                return;
            } else {
    ...
    protected void getDataFile(XDATUser user, String path, HttpServletResponse response) throws ServletException, IOException {
        String rootElementName = path.substring(0, path.indexOf("/"));
        path = path.substring(path.indexOf("/") + 1);
    ...
    protected void doGetOrPost(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException {
        ...
        if (user == null) {
            user = (XDATUser) req.getSession().getAttribute("user");
        }
