...
public class XnatDicomseriesBean extends XnatAbstractresourceBean implements java.io.Serializable, org.nrg.xdat.model.XnatDicomseriesI {
	public static org.apache.log4j.Logger logger = org.apache.log4j.Logger.getLogger(XnatDicomseriesBean.class);
	...
	public void setOrientation(String v){
	 ...
	 private List<org.nrg.xdat.bean.XnatDicomseriesImageBean> _Imageset_image =new ArrayList<org.nrg.xdat.bean.XnatDicomseriesImageBean>();

	/**
	 * imageSet/image
	 * @return Returns an List of org.nrg.xdat.bean.XnatDicomseriesImageBean
	 */
	 ...
	 * @param v Value to Set.
	 */
	public void setImageset_image(ArrayList<org.nrg.xdat.bean.XnatDicomseriesImageBean> v){
		_Imageset_image=v;
	}

	/**
	 * Adds the value for imageSet/image.
	 * @param v Value to Set.
	 */
	public void addImageset_image(org.nrg.xdat.bean.XnatDicomseriesImageBean v){
		_Imageset_image.add(v);
	}
	...
	_Imageset_image.add((org.nrg.xdat.bean.XnatDicomseriesImageBean)item);
	}
	 ...
	 * @param v Value to Set.
	 */
	public void addImageset_image(Object v){
		if (v instanceof org.nrg.xdat.bean.XnatDicomseriesImageBean)
			_Imageset_image.add((org.nrg.xdat.bean.XnatDicomseriesImageBean)v);
		else
			throw new IllegalArgumentException("Must be a valid org.nrg.xdat.bean.XnatDicomseriesImageBean");
	}
	...
	public void setReferenceField(String xmlPath,BaseElement v) throws BaseElement.UnknownFieldException{
		if (xmlPath.equals("imageSet/image")){
			addImageset_image(v);
		}
		else{
	...
	public String getFieldType(String xmlPath) throws BaseElement.UnknownFieldException{
		if (xmlPath.equals("dimensions/x")){
			...
			return BaseElement.field_data;
		}else if (xmlPath.equals("imageSet/image")){
	...
	protected TreeMap getXMLAtts() {
		...
		else map.put("UID","");//REQUIRED FIELD
	...
	protected boolean addXMLBody(java.io.Writer writer, int header) throws java.io.IOException{
		...
		//REFERENCE FROM dicomSeries -> abstractResource
		...
		if (_Orientation!=null){
			...
			int child2=0;
			int att2=0;
			child2+=_Imageset_image.size();
			if(child2>0 || att2>0){
				writer.write("\n" + createHeader(header++) + "<xnat:imageSet");
			if(child2==0){
				writer.write("/>");
			}else{
				writer.write(">");
		//REFERENCE FROM dicomSeries -> imageSet/image
		java.util.Iterator iter3=_Imageset_image.iterator();
		while(iter3.hasNext()){
			org.nrg.xdat.bean.XnatDicomseriesImageBean child = (org.nrg.xdat.bean.XnatDicomseriesImageBean)iter3.next();
			writer.write("\n" + createHeader(header++) + "<xnat:image");
			child.addXMLAtts(writer);
			if(!child.getFullSchemaElementName().equals("xnat:dicomSeries_image")){
	...
	protected boolean hasXMLBodyContent(){
		...
		if (_Orientation!=null) return true;
			if(_Imageset_image.size()>0)return true;
